...
RSpec.feature "Purchase Product", type: :feature do
  ...
  scenario "Verify gifting fails if child isn't in the system" do
    product = Product.create!(
      name: "product1",
      description: "description2",
      price_cents: 1000,
    ...
    fill_in "order[expiration_year]", with: 25
    fill_in "order[shipping_name]", with: "Pat Jones"
    fill_in "order[giver_name]", with: "Uncle Bob"
    fill_in "order[child_full_name]", with: "Kim Jones"
    fill_in "order[child_birthdate]", with: "2019-03-03"
  ...
  scenario "Verify gifting reuses earlier order's address" do
    product = Product.create!(
      name: "product1",
      description: "description2",
      price_cents: 1000,
    ...
    gift_order = Order.last

    expect(original_order.address).to eq(gift_order.address)
    expect(original_order.zipcode).to eq(gift_order.zipcode)
  end
end
